...
const useAuthCall = () => {
  ...
  const login = async (userData) => {
    // const BASE_URL = "http://14192.fullstack.clarusway.com";
    ...
    try {
      const { data } = await axios.post(
        `https://14192.fullstack.clarusway.com/account/auth/login/`,
        userData
      );
      dispatch(loginSuccess(data));
      toastSuccessNotify("login islemi basarili");
  ...
  const logout = async () => {
    ...
    try {
      await axios.post(
        `https://14192.fullstack.clarusway.com/account/auth/logout/`
      );
  ...
  const register = async (userData) => {
    ...
    try {
      const { data } = await axios.post(
        ...
        `https://14192.fullstack.clarusway.com/account/register/`,
        userData
      );
